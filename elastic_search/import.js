...
if(process.argv.length !== 12) {
    console.log('USAGE: ', process.argv[0], process.argv[1], 'originHost originESIndex originESType targetHost targetESIndex targetESType from size throttleInMs searchFilter');
    process.exit(1);
}

var originHost = process.argv[2];
var originESIndex = process.argv[3];
var originESType = process.argv[4];
var targetHost = process.argv[5];
...
console.log("FROM: Elastic Search host:", originHost);
console.log("TO:   Elastic Search host:", targetHost);
...
setTimeout(function() {
    ...
    function scanAndScroll(next) {
        ...
        esOrigin.search({index: originESIndex, type: originESType, body: matchAllFilter, searchType: 'scan', scroll: '10s'}, function getMoreUntilDone(err, scanSearch, status) {
            if (err) {
                console.log('Error while scanning data from Elastic Search host', originHost, 'for index', originESIndex, 'and type', originESType, ', error:', err, ', status:', status);
                process.exit(1);
            }
    ...
    function reindex(bulkBody) {
      function bulk(bulkBodyPortion) {
          esTarget.bulk({body: bulkBodyPortion}, function(err, response, status) {
              if (err) {
                  console.log('Error while indexing data to Elastic Search host', targetHost, 'for index', targetESIndex, 'and type', targetESType, ', error:', err, ', status:', status);
                  process.exit(1);
              }
              reindexFrom += size*2;
              reindex(bulkBody);
      ...
      if(bulkBody.length === 0 || reindexFrom>=(bulkBody.length)) {
          console.log('' + bulkBody.length/2, 'documents indexed to Elastic Search host', targetHost, 'for index', targetESIndex, 'and type', targetESType, '. Total time,', new Date() - startTime, 'ms');
          process.exit(0);
      }
      setTimeout(function() {
